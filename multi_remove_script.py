#!/usr/bin/env python3
"""
Скрипт для рекурсивного удаления <script> тегов из HTML файлов
"""
...
LOG_FILE = LOG_DIR / "remove_script_tags.log"
...
logger = logging.getLogger("script_remover")

# Компиляция регулярного выражения заранее
SCRIPT_PATTERN = re.compile(r'<script\b[^>]*>.*?</script>', re.IGNORECASE | re.DOTALL)


def remove_script_tags_from_file(file_path):
    """
    Удаляет <script>...</script> теги из указанного файла.
    ...
    try:
        ...
        if count > 0:
            ...
            logger.info(f"Удалено {count} <script> тегов из файла: {file_path}")
        else:
            logger.info(f"<script> теги не найдены в файле: {file_path}")
...
def main():
    ...
    print("=" * 60)
    print("Удаление <script> тегов из HTML файлов")
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {
            executor.submit(remove_script_tags_from_file, file): file
